...
import (
	...
	"github.com/ercusz/go-bookings/internal/driver"
...
func TestRepository_PostReservation(t *testing.T) {
	reqBody := "start_date=1 January 2050"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2 January 2050")
...
func TestRepository_PostAvailability(t *testing.T) {
	...
	*****************************************/
	...
	reqBody := "start=1 January 2050"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end=2 January 2050")
	...
	*****************************************/
	// this time, we specify a start date before 1 January 2040, which will give us
	// a non-empty slice, indicating that rooms are available
	reqBody = "start=1 January 2040"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end=2 January 2040")
	...
	*****************************************/
	// this time, we specify a start date of 1 January 2060, which will cause
	// our testdb repo to return an error
	reqBody = "start=1 January 2060"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end=2 January 2060")
	req, _ = http.NewRequest("POST", "/search-availability", strings.NewReader(reqBody))
...
func TestRepository_BookRoom(t *testing.T) {
	...
	*****************************************/
	...
	req, _ := http.NewRequest("GET", "/book-room?s=1 January 2050&e=2 January 2050&id=1", nil)
	ctx := getCtx(req)
	req = req.WithContext(ctx)
	...
	*****************************************/
	req, _ = http.NewRequest("GET", "/book-room?s=1 January 2040&e=2 January 2040&id=4", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
